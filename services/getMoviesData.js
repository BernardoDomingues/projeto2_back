...
const getMoviesData = async (reqType, options) => {
  ...
  return axios
    .get(typeOfRequisitions[reqType]) // Faz a busca através do método fetch, acessando a URL através do objeto typeOfRequisitions com a chave passada por parâmetro na chamada da função
    .then((data) => {
      let { errorMessage } = data.data; // Captura a mensagem de erro vinda da API
      ...
      if (
        ...
        counter++;
        getMoviesData(reqType, options); // Retorna a própria função recursivamente
      }
      return data.data; // Retorna os dados da busca
    });
};
